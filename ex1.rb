...
def amount_violationcategory(a)
	keeptrack = []
	a.each do |x|
		if !keeptrack.include?(x[:violation_category])
			x[:violation_category] = []
			x[:violation_category].push(1)
			keeptrack.push(x[:violation_category])
		elsif keeptrack.include?(x[:violation_category])
			x[:violation_category].push(1)
		end
	end
	return keeptrack
...
puts amount_violationcategory(violations)